...
def total_distance(gpx):
    ...
    for i in range(len(gpx) - 1):
        ossz += position_distance(gpx[i]["position"], gpx[i+1]["position"])
    return ossz
...
def total_time(gpx):
    running = 0
    for i in range(len(gpx) - 1):
        running += (gpx[i+1]["timestamp"] - gpx[i]["timestamp"])
    return running
...
def idle_time(gpx):
    ossz = 0
    for i in range(len(gpx) - 1):
        if (gpx[i]["position"][0] == gpx[i+1]["position"][0]) and (gpx[i]["position"][1] == gpx[i+1]["position"][1]): 
            ossz += (gpx[i+1]["timestamp"] - gpx[i]["timestamp"])
    return ossz
...
def total_ascent(gpx):
    ...
    for i in range(len(gpx) - 1):
        if gpx[i]["elavation"] < gpx[i+1]["elavation"]: 
            emelkedes += (gpx[i+1]["elavation"] - gpx[i]["elavation"])
    return emelkedes
...
def chop_after_distance(gpx, distance):
    if total_distance(gpx) < distance: return []
    else:
        total = 0
        track = []
        for i in range(len(gpx) - 1):
